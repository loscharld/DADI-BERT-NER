...
class SkipGram:
    def __init__(self):
        self.data_index = 0
        # self.trainpath = './data1/train_data.txt'
        ...
        self.num_steps = 120000#定义最大迭代次数，创建并设置默认的session，开始实际训练
    ...
    def save_embedding(self, final_embeddings, reverse_dictionary):
        f = open(self.modelpath, 'w+',encoding='utf-8')
        for index, item in enumerate(final_embeddings):
            if index==0:
                f.write(str(len(final_embeddings)-1)+' '+str(self.embedding_size)+'\n')
            else:
