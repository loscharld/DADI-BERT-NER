...
def predict_yy():
    ...
    with graph.as_default():
        print(id2label)
        input_path='Data/CHEXI_NER_Data/test'
        with open('Data/CHEXI_NER_Data'+'/'+'xiaoshouku_result.txt','w',encoding='utf-8') as writer:
            Dir_list=os.listdir(input_path)
            for path in Dir_list:
                path1=input_path+'/'+path
                datas=pd.read_csv(path1)
                for i in range(len(datas)):
                    content = str(datas['STD_MODEL_CONCAT'][i])
                    start = datetime.now()
                    if len(content) < 2:
                        print(content)
                        continue
                    sentence = tokenizer.tokenize(content)
                    # print('your input is:{}'.format(sentence))
                    input_ids, input_mask, segment_ids, label_ids = convert(sentence)
                    ...
                    #todo: 组合策略
                    result = strage_combined_link_org_loc(sentence, pred_label_result[0], True)
                    writer.write(content+'\t'+str(result)+'\n')
                    print('识别的实体有：{}'.format(','.join(result)))
                    print('Time used: {} sec'.format((datetime.now() - start).seconds))
