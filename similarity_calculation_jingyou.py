...
class Doc_most_sim():
    def __init__(self):
        ...
        self.SynonymDict = {i.strip().split(';')[0]: i.strip().split(';')[1] for i in
                            open(self.SynonymPath, encoding='utf-8') if i.strip()}
    ...
    def synonym_exchange(self,sentence):
        synonym_dict = {}
        synonym = [i.strip() for i in open('./data1/车系dict.txt') if i.strip()]
        for s in synonym:
    ...
    def extract_most_sim_content_all(self, sentences,ff):
        ...
        for index, sentence in enumerate(sentences):
            ...
            senti_words = [r for r in senti_words if r != '' and r != '.']
                                ...
                                if len(banxings):
                                    for bx in banxings:
                                        if year in content and fuel_consumption in content and bx in content and lwh in content\
                                                and biansu in senti_words_biansu_content:
...
if __name__=='__main__':
    ...
    # for line in ['一汽奥迪 奥迪Q3 2018款 35 TFSI 2.0T 双离合 两驱 30周年纪念版  时尚型前置前驱 4398*1841*1591 DCT 5门5座2.0T国IV(国V)']:
    #     content =line.strip()
    #     start_time = datetime.now()
    #     content=content.upper()
    #     result=handle.doc_score_all(content)
